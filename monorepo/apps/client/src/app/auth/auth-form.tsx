...
import { signUp, signIn, selectAuthenticated } from '../../store/features/user/user-slice'
import { AuthHocState } from '../../components/auth-hoc'
...
function AuthForm({authHocState}: {authHocState: AuthHocState}) {
...
//   }, [pathname, params, searchParams] );    
  ...
  function getValidation(): any {
    ...
    if (authHocState.isSignIn) {
      validationFields = new AuthUserReqDto()
    }

    if (authHocState.isSignUp) {
  ...
  function validationOnSubmit(): boolean {
    ...
    if (authHocState.isSignUp) {
      if (!validations.length && authHocState.isSignUp && validationFields.password !== validationFields.pwdRetype) {
        allGood = false
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    dispatch<any>((authHocState.isSignUp ? signUp : signIn)(authUserReqDto))
    .unwrap()
    ...
    setAuthState((prevState: AuthState) => {
      ...
      if (fieldName === 'pwdRetype') {
        ...
        if (validationItem) {
          ...
          if (authHocState.isSignUp && fieldName === 'password') {
      ...
      <main className='container is-family-primary'>
        ...
        <section className='hero'>
          <div className='hero-body'>
            <p className='title'>
              {authHocState.isSignUp ? 'Sign up' : 'Log in'}
            </p>
            <p className='subtitle'>
              {authHocState.isSignUp ? 'Create account' : 'With username and password'}
            </p>
          </div>
        </section>
        ...
        <section className='mt-6'>
          <div className='columns is-mobile is-centered'>
            <div className='column is-one-third'>
                  ...
                  <div className='card-content'>    
                    <form onSubmit={onSubmit}>
                      {Object.keys(authState.fields).map((key) => {
                        ...
                        if (authHocState.isSignIn && key === 'pwdRetype') {
                      ...
                      <div className='field' >
                        <button className={`button is-primary`} disabled={authState.isLoading}>{authHocState.isSignUp ? 'Sign up' : 'Log in'}</button>
                      </div>             
                      {/* { 
                        (user && user.token) &&
              ...
              <div className='is-one-third'>
                {
                  authHocState.isSignUp && <h1>Already have an account? Log in</h1>
                }
                {
                  authHocState.isSignIn && <h1>Need a new account? Sign up</h1>
                }                
...
export default connect(selectAuthenticated)(AuthForm)